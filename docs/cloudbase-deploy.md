...
- 已创建 CloudBase 环境
- 环境 ID，例如 `xxx-123456`
- 一个管理员令牌，例如随机字符串 `survey-admin-2026-xxxx`

## 方式 A：使用 CloudBase CLI 登录

```bash
npx cloudbase login
```

如果当前终端无法弹出登录，请在本机 PowerShell 中手动执行。
...
```powershell
.\scripts\deploy-cloudbase.ps1 -EnvId 你的环境ID -AdminToken 你的管理员令牌
```

如果自动推断的云函数 HTTP 地址不对，可以手动指定：

```powershell
.\scripts\deploy-cloudbase.ps1 -EnvId 你的环境ID -AdminToken 你的管理员令牌 -ApiBase "https://你的HTTP访问域名"
```
...
- 如果 `GET /api/submissions` 返回 401：说明管理员令牌未输入或输入错误
