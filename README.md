...
- GitHub 仓库首页：<https://github.com/sheika8673338-ai/smart-questionnaire-system>
...
- GitHub 仓库页面用于展示源码与文档，不等同于在线运行页面
- 如果直接在 GitHub 中查看 `index.html` 文件，看到的只是源码预览，不是完整部署后的前端效果
- 需要体验真实界面时，请优先访问 CloudBase 线上地址或 GitHub Pages 地址
- 仓库已补充 GitHub Pages 工作流，后续可直接将 `main` 分支的静态前端发布为独立演示页

## 演示地址说明

这个项目存在两种“打开方式”，用途不同：

- GitHub 仓库：用于查看源码、README、提交记录和项目文档
- 在线部署地址：用于实际运行系统，加载样式、脚本和配置文件

因此，GitHub 页面里看到的 `index.html` 文件预览，和真正部署后的系统界面可能不一致。这是正常现象，不代表代码未更新。
